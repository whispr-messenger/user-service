...
describe('BlockedUsersService', () => {
	...
	describe('blockUser', () => {
		...
		it('should throw BadRequestException if trying to block self', async () => {
			...
			await expect(service.blockUser(mockUser.id, blockUserDto)).rejects.toThrow(BadRequestException);
		});
		...
		it('should throw NotFoundException if blocked user not found', async () => {
			...
			await expect(service.blockUser(mockUser.id, blockUserDto)).rejects.toThrow(NotFoundException);
		});
		...
		it('should throw ConflictException if user already blocked', async () => {
			...
			await expect(service.blockUser(mockUser.id, blockUserDto)).rejects.toThrow(ConflictException);
		});
	});
	...
	describe('unblockUser', () => {
		it('should unblock a user successfully', async () => {
			...
			expect(mockBlockedUserRepository.remove).toHaveBeenCalledWith(mockBlockedUserEntity);
		});

		it('should throw NotFoundException if blocked user not found', async () => {
			mockBlockedUserRepository.findOne.mockResolvedValue(null);

			await expect(service.unblockUser(mockUser.id, 'nonexistent')).rejects.toThrow(NotFoundException);
		});
	});

	describe('getBlockedUsers', () => {
		it('should return blocked users', async () => {
			mockBlockedUserRepository.findAndCount.mockResolvedValue([[mockBlockedUserEntity], 1]);
