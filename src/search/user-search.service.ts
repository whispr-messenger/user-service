...
export class UserSearchService {
	 ...
	 */
	async searchByName(query: string, options: SearchOptions = {}): Promise<UserSearchResult[]> {
		try {
			...
			if (users.length < limit) {
				...
				if (dbUsers.length > 0) {
					await Promise.all(dbUsers.map((u) => this.searchIndexService.indexUser(u)));
				}
			}

			// Format results with privacy filtering
			const resultPromises = users.map((user) => this.formatUserSearchResult(user, options.viewerId));
	...
	async advancedSearch(
		...
		try {
			...
			if (users.length > 0) {
				await Promise.all(users.map((u) => this.searchIndexService.indexUser(u)));
			}
	 ...
	 */
	async getSearchSuggestions(query: string, options: SearchOptions = {}): Promise<string[]> {
		try {
	...
	async rebuildSearchIndexes(): Promise<void> {
		try {
			...
			this.logger.log('Search index rebuilt using SearchIndexService batch utilities');
		} catch (error) {
	 ...
	 */
	private async formatUserSearchResult(user: User, viewerId?: string): Promise<UserSearchResult> {
