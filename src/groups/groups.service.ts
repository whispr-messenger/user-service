...
export class GroupsService {
	...
	async addMember(
		...
		try {
			...
			this.logger.log(`User ${addMemberDto.userId} added to group ${groupId} by ${addedById}`);
			return savedMember;
		} catch (error) {
			await queryRunner.rollbackTransaction();
	 ...
	 */
	async removeMember(groupId: string, memberId: string, removedById: string): Promise<void> {
		const queryRunner = this.dataSource.createQueryRunner();
		await queryRunner.connect();
		await queryRunner.startTransaction();
		...
		try {
			...
			this.logger.log(`User ${memberId} removed from group ${groupId} by ${removedById}`);
		} catch (error) {
			await queryRunner.rollbackTransaction();
	...
	async updateMemberRole(
		...
		const canUpdate = updater.role === GroupRole.ADMIN || group.createdBy.id === updatedById;
	...
	async getGroupMembers(
		...
		const queryBuilder = (this.groupMemberRepository as any).createQueryBuilder('member');
	...
	async deleteGroup(groupId: string, userId: string): Promise<void> {
		...
		try {
			...
			const canDelete =
				group.createdBy.id === userId || (userMember && userMember.role === GroupRole.ADMIN);
