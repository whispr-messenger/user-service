...
describe('GroupsService', () => {
	...
	describe('createGroup', () => {
		...
		it('should throw NotFoundException if user not found', async () => {
			...
			await expect(service.createGroup(createGroupDto, userId)).rejects.toThrow(NotFoundException);
		});
	});
	...
	describe('findGroupById', () => {
		...
		it('should throw NotFoundException if group not found', async () => {
			...
			await expect(service.findGroupById('nonexistent')).rejects.toThrow(NotFoundException);
		});
	});
	...
	describe('findUserGroups', () => {
		it('should return groups for a user', async () => {
			...
			expect(mockQueryBuilder.where).toHaveBeenCalledWith('member.userId = :userId', {
				userId: mockUser.id,
			});
		});
	});
	...
	describe('updateGroup', () => {
		it('should update a group successfully', async () => {
			...
			const result = await service.updateGroup(mockGroup.id, updateGroupDto, mockUser.id);
		...
		it('should throw ForbiddenException if user is not group creator', async () => {
			...
			await expect(service.updateGroup(mockGroup.id, updateGroupDto, differentUser)).rejects.toThrow(
				ForbiddenException
			);
		});
	});
	...
	describe('addMember', () => {
		it('should add a member to the group successfully', async () => {
			...
			const result = await service.addMember(mockGroup.id, addMemberDto, mockUser.id);
		...
		it('should throw ConflictException if user is already a member', async () => {
			...
			await expect(service.addMember(mockGroup.id, addMemberDto, mockUser.id)).rejects.toThrow(
				ConflictException
			);
		});
	});
	...
	describe('removeMember', () => {
		it('should remove a member from the group successfully', async () => {
			...
			expect(mockGroupMemberRepository.remove).toHaveBeenCalledWith(mockGroupMember);
		});
		...
		it('should throw NotFoundException if member not found', async () => {
			...
			await expect(service.removeMember(mockGroup.id, 'nonexistent-user', mockUser.id)).rejects.toThrow(
				NotFoundException
			);
		});
	});
	...
	describe('updateMemberRole', () => {
		it('should update member role successfully', async () => {
			...
			const result = await service.updateMemberRole(mockGroup.id, mockUser.id, newRole, mockUser.id);
	...
	describe('deleteGroup', () => {
		...
		it('should throw ForbiddenException if user is not group creator', async () => {
			...
			await expect(service.deleteGroup(mockGroup.id, 'different-user')).rejects.toThrow(
				ForbiddenException
			);
		});
	});
	...
	describe('searchGroups', () => {
		it('should search groups by name', async () => {
			...
			expect(result.groups).toEqual([mockGroup]);
			expect(result.total).toBe(1);
			expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('LOWER(group.name) LIKE LOWER(:query)', {
				query: '%Test%',
			});
		});
	});
